...
// removed from the scene, same as populateFurnitureShape. Fits to `height` (the structure's own,
// now-real field - see shared's BUILDING_TARGET_HEIGHT/StructureDef.height doc comments) rather
// than a fixed per-model constant, so a resized building actually renders at its new size.
export function populateBuildingShape(group: THREE.Group, modelId: string, height: number): void {
